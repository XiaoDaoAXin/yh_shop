<template>
  <div class="index" >
    ...
    <header>
      <!--搜索search区域-->
      <Header type="routers" right="choice" title="多门店" middle="title" />
      <!--nav导航区域-->
      <section class="h_nav">
        <div class="h_nav_div">
          <!--横向滑动-->
    ...
    <div id="index">
      <div id="index_box">
        ...
        <section class="btns">
          ...
          <!--<img src="https://gw.alicdn.com/tps/TB155AUPpXXXXajXVXXXXXXXXXX-1125-480.png" style="width: 100%;" />-->
          <!--</div>-->
        ...
        <div @click="$router.replace('/stores/integral')">
          <img src="../../../../static/img/11.png" style="width: 100%;" />
        </div>
        <!--商品列表-->
        <!--<keep-alive>
...
<script>
  ...
  import StoresShops from "../../../components/StoresShops/StoresShops.vue";
  ...
  export default {
    ...
    created() {
      ...
      this.sessionIndexDate = JSON.parse(sessionStorage.getItem("assemble"))
      this.pullUpLoadP = JSON.parse(sessionStorage.getItem("pullUpLoadPAssemble"))       //取到本地存储的数据
      if(this.sessionIndexDate){
        ...
        this.$store.dispatch('getAssemble');
        this.loadding = true
      }
      this.$nextTick(() => {
        this.windowScroll();
    ...
    methods: {
      ...
      windowScroll(){
        ...
        this.scrolls.on("pullingUp",() => {
          this.sessionIndexDate = JSON.parse(sessionStorage.getItem("assemble"))              //取到本地存储的数据
          this.pullUpLoadP = JSON.parse(sessionStorage.getItem("pullUpLoadPassemble"))       //取到本地存储的数据
          if(this.pullUpLoadP){
            this.$store.dispatch('getAssemble',this.pullUpLoadP)      //请求已缓存的下一组数据
          }
          this.pullUpLoadP++;       //如果此数据有值，将页数+1
        });
        //滚动监视
        let scroll_record = JSON.parse(sessionStorage.getItem("scrollAssemble"))       //取到本地存储的数据
        this.scrolls.on("scroll",(obj) => {
          if(obj.y){
            this.sessionIndex("scrollAssemble",obj.y);                                     //缓存滚动值
          }
        });
        //滚动条滚动到原来的位置
        ...
        this.scrolls.on("pullingDown",() => {
          this.sessionIndex("assemble","");               //初始化缓存首页数据
          this.sessionIndex("pullUpLoadPAssemble",1);    //初始化缓存页数
          this.$store.dispatch('getAssemble');           //初始化请求
    ...
    watch:{
      assemble(val){
        if(!this.sessionIndexDate){
          this.$router.replace({path: "/homeHref",query:{path: '/assemble'}})         //刷新页面
          this.loadding = false;
          this.navTab = val.nav
          this.navFn(val);
          console.log(1)
          this.sessionIndex("assemble",val);             //初始化缓存
          this.sessionIndex("pullUpLoadPAssemble",2);  //初始化缓存
        }else{
          ...
          if(val.hot_goods.length){                 //如果请求有数据，就执行
            ...
            if(this.booleanRefresh){                //判断是否是刷新
              ...
              /*this.booleanRefresh = false;          //关闭刷新按钮
              this.booleanData =  false,            //初始化是否是已经记载完数据
              this.bottomLoading = true,            //初始化底部加载
              this.booleanRefresh = false           //初始化是否刷新*/
              setTimeout(() => {
                Toast({
                  message:"加载完毕！",
                  duration:1500
            ...
            this.sessionIndex("pullUpLoadPAssemble",this.pullUpLoadP);                   //初始化缓存
          }else{
          ...
          this.sessionIndex("assemble",this.sessionIndexDate);     //将已处理的数据缓存到本地种
        }
