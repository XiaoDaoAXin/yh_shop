  ...
  <section class="searchShops">
          ...
          <div class="tabBtn_screen_box animated fadeInRightBig">
            ...
            <section class="tabBtn_screen_box_list">
              <yd-accordion accordion>
                <yd-accordion-item title="全部分类" style="font-size: 26px;">
                  <ul>
                ...
                <yd-accordion-item :title="spec.name" style="font-size: 26px;" v-for="(spec,index) in specifications" :key="index">
                  <ul>
    ...
    <section class="shops" v-if="shops">
      ...
      <ul class="shops_vertical" v-else>
        <li class="shops_vertical_li" v-for="(shop , index) in shops" :key="index" @click="shopsdetails(shop.id)">
          <!--商品图片-->
          <img class="shops_vertical_li_img" :src="url+shop.original_img" height="200" />
          <!--商品标题-->
...
<script>
  ...
  export default {
    ...
    data(){
      return{
        shops: [],
        listOption: this.$route.query.listOption||false,     //列表选项
        value: String,         //搜索内容
        url: URL,
        loading: true,
